...
if [[ "${UID}" -ne 0 ]]
then
    echo "${LOGGED_IN_USER_NAME}, isn't authorised to execute this script."
    exit 1
fi
...
if [[ "${#}" -lt 1 ]]
then
    echo "Insuffient information provided to the script as arguments. Please refer to the usage of this script."
    echo "script USER_ACCOUNT_NAME [USER_ACCOUNT_COMMENT]... "
    exit
fi
ACCOUNT_NAME=${1}
shift
...
# Create user account
useradd -c "${ACCOUNT_COMMENT}" -m "${ACCOUNT_NAME}"

# Make the user account to change the password on first login
if [[ "${?}" -ne 0 ]]
then 
    echo "Some issue with the script. Please contact the author."
fi

echo "${PASSWORD}" | passwd --stdin "${ACCOUNT_NAME}"
if [[ "${?}" -ne 0 ]]
then 
    echo "Some issue with the script. Please contact the author."
fi
